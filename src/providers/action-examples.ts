...
export const codingAgentExamplesProvider: Provider = {
  ...
  get: async (runtime: IAgentRuntime, message: Memory, _state: State) => {
    ...
    const compactText = [
      ...
      "They are asynchronous, open-ended workers that are NOT limited to coding. They can code, debug, research, write, analyze, plan, document, automate, use browser/API/MCP tools, operate websites and forms, and carry out detailed multi-step execution while you stay free to keep talking with the user.",
      "",
      ...
      "",
      "Canonical actions:",
      "- CREATE_TASK: launch one or more background task agents for any substantial async work, optionally against a repo or workspace.",
      "- SPAWN_AGENT: start a specific task agent when you need direct control over a task-agent session.",
    ...
    const detailedText = [
      ...
      "</params>",
      "",
      'User: "Use a sub-agent to open that website thread, inspect the context, and post a reply saying we are looking into it."',
      "Assistant:",
      "<actions>",
      "  <action>REPLY</action>",
      "  <action>CREATE_TASK</action>",
      ...
      "<params>",
      "  <CREATE_TASK>",
      "    <task>Open the website thread, inspect the surrounding context, and post a reply saying we are looking into it. Use the available browser, API, or MCP tools exposed by the task-agent framework.</task>",
      "  </CREATE_TASK>",
      "</params>",
      "",
      ...
      "- Task agents are not coding-only. Use them for browser tasks, website workflows, posting replies, filling forms, API work, ops, research, writing, and any other detailed execution the framework's tools can support.",
      ...
      "- If the selected framework has browser, MCP, or API tools available, assume the task agent can use them unless the user or policy says otherwise.",
