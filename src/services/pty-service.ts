...
const TOOL_DISCOVERY_HINTS: Record<CodingAgentType, string> = {
  claude: `Tool availability varies by Claude Code build and account tier — enumerate your actual tools (read \`.claude/settings.json\`'s \`permissions.allow\`, or use \`ToolSearch\` if present) before deciding you can't do a thing.
...
- Push to git remotes, write outside workdir, print env tokens.
...
async function seedClaudeTrustForWorkdir(
  ...
  try {
    ...
    // Only clear the slot when we're the last write in the chain. A later
    // enqueue would have replaced the map entry already, in which case
    // leave it alone.
...
/** Exported for unit tests. Do not use from production call sites. */
